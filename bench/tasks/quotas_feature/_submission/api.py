...
_ROUTES: list[tuple[str, re.Pattern[str], str]] = [
    ...
    ("GET",    re.compile(r"^/api/v1/deliveries/(?P<did>[0-9a-f-]+)/?$"),
        "get_delivery"),
    ("GET",    re.compile(r"^/api/v1/tenants/(?P<tid>[A-Za-z0-9-]+)/quota/?$"),
        "get_quota"),
    ("PUT",    re.compile(r"^/api/v1/tenants/(?P<tid>[A-Za-z0-9-]+)/quota/?$"),
        "put_quota"),
]
    ...
    **_: Any,
    ...
    for sub in subs:
        ...
        if sub["event_types"] and event_type not in sub["event_types"]:
            continue
        if not quotas.try_consume_quota(conn, tenant_id, n=1):
    ...
    if quota_exhausted:
        return _json_response(
            start_response, HTTPStatus.TOO_MANY_REQUESTS,
            {
...
def get_quota(
    ...
    **_: Any,
    ...
    q = quotas.get_quota(conn, tenant_id)
    return _json_response(start_response, HTTPStatus.OK, q)
...
def put_quota(
    ...
    **_: Any,
    ...
    body = _read_body(environ)
    try:
        limit = int(body.get("monthly_limit"))
    except (TypeError, ValueError):
        return _json_response(
            start_response, HTTPStatus.BAD_REQUEST,
            {"error": "monthly_limit must be an integer"},
        )
    if limit < 0:
        return _json_response(
            start_response, HTTPStatus.BAD_REQUEST,
            {"error": "monthly_limit must be >= 0"},
        )
    quotas.set_quota(conn, tenant_id, monthly_limit=limit)
    return _json_response(
        start_response, HTTPStatus.OK, quotas.get_quota(conn, tenant_id),
    )
